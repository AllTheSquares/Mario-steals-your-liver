...
def standing_Mario(): # Swaps initial image of Mario kicking a hole through your screen to him standing asking for your liver
    ...
    marioStand = PhotoImage(file = 'Contents\marioStand.png') # Pulls image from contents folder
    marioStand = marioStand.zoom(2,2) # Enlargens image

    canvas.delete(Mario) # Deletes original instance of Mario (kicking pose)
    canvas.create_image(canvasWidth/2,canvasHeight/2, image = marioStand, anchor=CENTER) # creates new canvas image at the same position of the original
    canvas.after(3500, hand_zoom)

def hand_zoom():
    global handFrame, handCanvas

    handCanvas = Canvas(handFrame, width = canvasWidth, height = canvasHeight)
    handCanvas.pack()

    img = Image.open("Contents\hand.gif") 
    hand = Label(handFrame) # Creates a label that will refresh per frame of the GIF.
...
def start_anim(): # called by the original script (liver.py) after a random amount of time
    ...
    hole = PhotoImage(file = "Contents\hole.png")
    marioKick = PhotoImage(file = "Contents\marioKick.png")

    # Adds the hole image AND Mario himself
    canvas = Canvas(root, width = canvasWidth, height = canvasHeight)
    canvas.create_image(canvasWidth/2,canvasHeight/2, image=hole, anchor=CENTER)
    Mario = canvas.create_image(canvasWidth/2,canvasHeight/2, image=marioKick, anchor=CENTER)
